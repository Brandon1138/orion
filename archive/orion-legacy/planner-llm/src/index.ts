...
// Legacy types for backward compatibility (Phase 1A DayPlan)
interface DayPlan {
	date: string;
	summary: string;
	blocks: PlanBlock[];
...
function validateDayPlan(plan: unknown): plan is DayPlan {
...
export class PlannerLLM {
	 ...
	 */
	private async generatePlanWithFallback(context: PlanningContext): Promise<DayPlan> {
		const systemPrompt = this.buildSystemPrompt();
		const userPrompt = this.buildUserPrompt(context);
	 ...
	 */
	private generateFallbackPlan(context: PlanningContext): DayPlan {
	 ...
	 */
	private getConversationInsights(input: TaskInterviewInput): {
		riskFactors: string[];
		...
		if (
			interviewState.completedTopics.length === 0 &&
	 ...
	 */
	async conductTaskInterview(input: TaskInterviewInput, sessionId?: string): Promise<TaskPlan> {
		try {
			...
			const taskPlan = parsedPlan as TaskPlan;
			...
			const enhancedNextSteps = this.generateIntelligentNextSteps(
				taskPlan,
				interviewState,
				insights
			);
	...
	private generateIntelligentNextSteps(
		taskPlan: TaskPlan,
		interviewState: { phase: string; completedTopics: string[]; questionsNeeded: number },
		insights: { riskFactors: string[]; optimizationSuggestions: string[]; readinessScore: number }
	): string[] {
	...
	private generatePriorityQuestions(
		...
		const urgentTasks = tasks.filter(
			task => task.due && new Date(task.due) <= new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
		);
	...
	private generateSchedulingQuestions(
		...
		const largeTasks = tasks.filter(
			task => task.title.length > 30 || (task.notes && task.notes.length > 50)
		);
	...
	private generateFollowUpQuestions(
		...
		if (conversationText.includes('deadline') || conversationText.includes('urgent')) {
			questions.push({
				question:
					'Are there any other deadlines I should know about that might affect task scheduling?',
				type: 'deadline',
				required: false,
			});
		}
	 ...
	 */
	private analyzeInterviewState(input: TaskInterviewInput): {
		phase: 'INIT' | 'PRIORITY' | 'CONTEXT' | 'SCHEDULING' | 'FOLLOWUP' | 'READY';
		...
		let phase: 'INIT' | 'PRIORITY' | 'CONTEXT' | 'SCHEDULING' | 'FOLLOWUP' | 'READY' = 'INIT';

		// Check what topics have been covered
		if (
			conversationText.includes('priority') ||
		...
		if (
			conversationText.includes('complex') ||
		...
		if (
			conversationText.includes('depend') ||
		...
		const questionsNeeded = Math.max(
			1,
			Math.min(3, 4 - Math.floor(conversationHistory.length / 2))
		);
	 ...
	 */
	private buildConversationalPrompt(input: TaskInterviewInput): string {
...
- Don't include markdown headings, comments, or narrative inside the JSON.`;
	 ...
	 */
	private buildTaskInterviewPrompt(input: TaskInterviewInput): string {
		...
		let suggestedQuestions: TaskQuestion[] = [];
...
`;
	 ...
	 */
	private validateTaskPlan(plan: unknown): plan is TaskPlan {
		...
		for (const analysis of p.taskAnalysis) {
			...
			if (
				typeof analysis.estimatedDuration !== 'number' ||
	 ...
	 */
	private generateFallbackTaskPlan(input: TaskInterviewInput): TaskPlan {
