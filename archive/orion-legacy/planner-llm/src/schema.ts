 ...
 */
export function validateDayPlan(plan: unknown): plan is DayPlanSchema {
...
function validatePlanBlock(block: unknown): boolean {
...
function validateAmbiguity(ambiguity: unknown): boolean {
	if (!ambiguity || typeof ambiguity !== 'object') {
		return false;
	}

	const a = ambiguity as Record<string, unknown>;
