...
import { createGoogleTasksAuth } from './google-auth.js';
export class TaskParser {
    ...
    hasGoogleCredentials() {
        return !!((process.env.GOOGLE_TASKS_CLIENT_ID || process.env.GOOGLE_CLIENT_ID) &&
     ...
     */
    async loadGoogleTasks(taskListIds) {
        if (!this.googleAuth) {
        ...
        if (targetListIds.length === 0) {
            ...
            if (listsResponse.data.items) {
                listsToProcess = listsResponse.data.items;
            }
        }
        else {
            ...
            for (const listId of targetListIds) {
                try {
                    ...
                    if (listResponse.data) {
                        listsToProcess.push(listResponse.data);
                    }
                }
                catch (error) {
     ...
     */
    normalizeGoogleTaskList(googleList) {
        return {
            id: googleList.id,
     ...
     */
    normalizeGoogleTask(googleTask, taskList) {
    ...
    buildTaskHierarchy(tasks) {
        ...
        const sortTasksRecursively = (taskList) => {
    ...
    applPrivacyMasking(tasks) {
        ...
        return tasks.map(task => {
            const isPrivate = privateKeywords.some(keyword => task.title.toLowerCase().includes(keyword.toLowerCase()) ||
