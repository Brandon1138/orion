...
export function SessionList({ sessions, onToggleCollapse, isCollapsed = false }: SessionListProps) {
	return (
			...
			<div className="flex items-center justify-between p-6 border-b border-border">
				<div className="flex items-center gap-2">
					...
					<button
						...
						title={isCollapsed ? 'Expand sessions (Ctrl/Cmd+B)' : 'Collapse sessions (Ctrl/Cmd+B)'}
			...
			<div className="flex-1 overflow-hidden p-3">
				<div className="scrollbar-custom h-full overflow-y-auto space-y-2">
					{sessions.map((session, index) => (
						<SessionItem key={session.id} session={session} isActive={index === 0} index={index} />
					))}
				</div>
			</div>
		</div>
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-3">
				...
				>
					<svg
						className={`h-4 w-4 ${isActive ? 'text-white' : 'text-foreground-muted'}`}
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
