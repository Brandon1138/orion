...
import { GoogleCalendarAuth } from './google-auth.js';
...
export class CalendarParser {
     ...
     */
    async loadGoogleEvents(dateRange) {
        if (!this.googleAuth) {
     ...
     */
    normalizeGoogleEvent(googleEvent, calendarId) {
     ...
     */
    mapGoogleResponseStatus(status) {
        switch (status) {
            case 'accepted':
                return 'yes';
            case 'declined':
     ...
     */
    mapGoogleVisibility(visibility) {
        switch (visibility) {
            case 'private':
                return 'private';
            case 'confidential':
