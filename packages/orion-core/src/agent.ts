...
export class TaskPlanningOrchestrator {
	 ...
	 */
	async conductTaskInterview(
		userMessage: string,
		sessionId?: string
	): Promise<{ taskPlan: TaskPlan | null; success: boolean; error?: string }> {
		try {
	 ...
	 */
	private validateTaskPlanStructure(plan: TaskPlan): boolean {
		try {
 ...
 */
export function createOrionAgent(config: OrionConfig): {
	name: string;
	instructions: string;
	model: string;
 ...
 */
export function createOrionContext(
	config: OrionConfig,
	sessionId?: string,
	userId?: string
): OrionContext {
...
export async function runOrionAgent(
	agent: { orchestrator: TaskPlanningOrchestrator },
	context: OrionContext,
	userMessage: string
): Promise<{ taskPlan: TaskPlan | null; response: string }> {
	try {
 ...
 */
function formatTaskPlanResponse(plan: TaskPlan | null, userMessage: string): string {
	if (!plan) {
