...
Generates a structured TaskPlan with priority analysis and calendar suggestions.`)
    ...
    async previewAndMaybeExecute(message, autoApproveLow, noPrompt = false) {
        if (!this.orion)
            return;
        console.log(chalk.blue('Intent and action preview (dry-run)'));
        const preview = await this.orion.previewActions(message);
        ...
        const { runSelection } = await inquirer.prompt([
            {
                type: 'list',
                name: 'runSelection',
        ...
        let actionsToRun = preview.actions;
            ...
            const { selected } = await inquirer.prompt([
                {
                    ...
                    choices: preview.actions.map((a, i) => ({
        ...
        const results = await this.orion.runActions(actionsToRun);
    ...
    async showStatusCommand() {
        ...
        try {
            const tools = await this.orion.listTools?.();
    ...
    handleDebugCommand(options) {
        ...
        if (options.memory) {
            ...
            console.log(chalk.gray('='.repeat(50)));
            try {
                const items = this.orion.getRecentMemory(this.sessionId, 20);
                if (!items || items.length === 0) {
                    ...
                    items.forEach((it, idx) => {
                        ...
                        console.log(`   ${JSON.stringify(it.data).slice(0, 200)}${JSON.stringify(it.data).length > 200 ? '...' : ''}`);
                    });
                }
            }
            catch (err) {
        ...
        if (options.actions) {
            ...
            console.log(chalk.gray('='.repeat(50)));
            console.log(chalk.gray('Use "orion chat --dry-run -m \"<message>\"" to generate an action preview.'));
        }
        ...
        try {
            ...
            try {
                ...
                if (taskPlanMatch) {
                    ...
                    const actions = this.orion.convertTaskPlanToActions(taskPlan);
                    if (actions && actions.length > 0) {
                        ...
                        if (session?.currentTaskPlan) {
                            this.displayTaskPlan(session.currentTaskPlan);
                            const actions = this.orion.convertTaskPlanToActions(session.currentTaskPlan);
                            if (actions && actions.length > 0) {
                                console.log('');
                                console.log(chalk.blue('Proposed Actions from TaskPlan:'));
                                actions.forEach((a, i) => {
    ...
    displayTaskPlan(taskPlan) {
        ...
        if (taskPlan.taskAnalysis && taskPlan.taskAnalysis.length > 0) {
            ...
            taskPlan.taskAnalysis.forEach((analysis, index) => {
                ...
                console.log(`   ${priorityColor(`Priority: ${analysis.priority}`)} | ${complexityEmoji} ${analysis.complexity} | Duration: ${analysis.estimatedDuration}min`);
